...
inline __host__ __device__ float2 make_float2(Eigen::Vector2f a) {
  return make_float2(a.x(), a.y());
}
...
inline __host__ __device__ float3 make_float3(Eigen::Vector3f a) {
  return make_float3(a.x(), a.y(), a.z());
}
...
inline __host__ __device__ float4 make_float4(const Eigen::Matrix4f &mat,
                                              int row) {
  Eigen::Vector4f r = mat.row(row);
...
inline __device__ __host__ float4 matVecMult(float4 *m, float4 v) {
  return make_float4(dot(m[0], v), dot(m[1], v), dot(m[2], v), dot(m[3], v));
}
