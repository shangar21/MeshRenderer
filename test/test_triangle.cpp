...
void testTriangleIntersection() {
    ...
    Ray ray(Eigen::Vector3f(0.25f, 0.25f, -1.0f), Eigen::Vector3f(0.0f, 0.0f, 1.0f));
    ...
    // Output results
    if (result) {
        std::cout << "Intersection detected at: " << hit.point.transpose() << std::endl;
        std::cout << "Intersection lambda (distance): " << hit.lambda << std::endl;
        std::cout << "Barycentric coordinates: " << triangle.getBarycentric(hit.point).transpose() << std::endl;
    } else {
